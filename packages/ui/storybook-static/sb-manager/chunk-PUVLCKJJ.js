...
Your code should look like: 
  const MyComponent = lazy(() => import('./MyComponent'))`,
      ...
      function getValueFromNode(node) {
        ...
        return (
          node &&
            (isCheckable(node) ? (value = node.checked ? 'true' : 'false') : (value = node.value)),
          value
        );
      }
...
`,
      ...
      function getData() {
        ...
        for (
          end2 = 1;
          end2 <= minEnd && startValue[startLength - end2] === endValue[endLength - end2];
          end2++
        );
...
Incoming: %s`,
      ...
      function mountTransition(config) {
        var _mountState2 = mountState(!1),
          ...
          start2 = mountCallback(startTransition.bind(null, setPending, config), [
            setPending,
            config,
          ]);
      ...
      function updateTransition(config) {
        var _updateState2 = updateState(),
          ...
          start2 = updateCallback(startTransition.bind(null, setPending, config), [
            setPending,
            config,
          ]);
        return [start2, isPending];
...
Learn more about data fetching with Hooks: https://fb.me/react-hooks-data-fetching`)
      ...
      function getHostSibling(fiber) {
        ...
        siblings: for (;;) {
          ...
          for (
            ...
            if (node.effectTag & Placement || node.child === null || node.tag === HostPortal)
              continue siblings;
            (node.child.return = node), (node = node.child);
          }
      ...
      function unmountHostComponents(finishedRoot, current2, renderPriorityLevel) {
        for (
          var node = current2, currentParentIsValid = !1, currentParent, currentParentIsContainer;
          ;
          ...
          if (node.tag === HostComponent || node.tag === HostText)
            ...
            (commitUnmount(finishedRoot, node, renderPriorityLevel), node.child !== null)
          ) {
            (node.child.return = node), (node = node.child);
            continue;
          }
