...
const validateWebhookPlugin = (
  plugin: SerializableWebhookPluginParams,
): void => {
  validateTransformers(plugin.name, plugin.request.transform)
};
...
const validateChildPlugin = (
  plugin: ISerializableChildPluginParams
): void => {
  plugin.transformers?.forEach(transform => {
    validateMapping(transform.transformer as Parameters<typeof validateMapping>[0], transform.mapping as Parameters<typeof validateMapping>[1])
  })
};
...
const validateMapping = (type: 'jmespath' | 'json-logic', mapping: string | Record<string, unknown>) =>{
