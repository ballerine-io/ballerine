...
We accept contributions in different sizes and varied difficulty, from documentation and typos, to features and bug fixes. To see what you could help with, report an issue, or suggest a feature see the following GitHub Actions workflows:
1. Building
2. Testing
3. Linting
4. Formatting.

Before creating a pull request, make sure that all of these workflows have passed. Before creating a pull request, make sure that all of these workflows have passed. To check the status of the workflows, navigate to the 'Actions' tab on the GitHub repository page.
...
<CodeBlock lang={`shell`} code={`node: ">=16.15.1"
pnpm: ">=7.11.0"`}/>
...
<CodeBlock lang={`shell`} code={`git clone git@github.com:ballerine-io/ballerine.git`}/>

2. Navigate to the directory and install dependencies

<PackageManagersTabs prefix={`cd ballerine &&`} code={`install`}/>
...
<PackageManagersTabs code={`run branchlint`}/>
...
<PackageManagersTabs code={`run commit`}/>

4. Push the commit

<CodeBlock lang={`shell`} code={`git push`}/>
