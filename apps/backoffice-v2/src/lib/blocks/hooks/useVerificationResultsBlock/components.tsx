...
interface CollapsibleSectionProps {
  title: string;
  defaultOpen?: boolean;
  statusBadge?: { text: string; variant: 'success' | 'destructive' | 'warning' | 'info' | 'violet' };
...
export const CollapsibleSection: FunctionComponent<CollapsibleSectionProps> = ({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="mt-3">
      <CollapsibleTrigger className="flex w-full items-center gap-2 rounded-lg bg-muted/50 px-4 py-2.5 text-left hover:bg-muted/80 transition-colors">
        <ChevronDown
          className={`h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-200 ${open ? '' : '-rotate-90'}`}
        />
        <span className="text-sm font-semibold">{title}</span>
        {statusBadge && (
          <Badge variant={statusBadge.variant} className="ml-auto text-[10px] px-1.5 py-0.5">
            {statusBadge.text}
          </Badge>
        )}
      </CollapsibleTrigger>
...
export const ReadOnlyDetailsInline: FunctionComponent<ReadOnlyDetailsInlineProps> = ({ details }) => {
...
export const AlertBanner: FunctionComponent<AlertBannerProps> = ({ level, message }) => {
  ...
  return (
    <div className={`flex items-center gap-2 rounded-lg border ${config.border} ${config.bg} px-4 py-3 mb-3`}>
      {level === 'success' && (
