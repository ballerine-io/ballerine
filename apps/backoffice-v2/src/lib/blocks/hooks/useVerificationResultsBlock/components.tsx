...
export const AlertBanner: FunctionComponent<AlertBannerProps> = ({ level, message }) => {
  ...
  return (
    ...
    >
      {level === 'success' && (
        <svg className={`size-5 ${config.text} shrink-0`} viewBox="0 0 20 20" fill="currentColor">
          <path
            fillRule="evenodd"
...
export const computeAlertLevel = (
  ...
  if (allVerified && statuses.filter(Boolean).length > 0) {
    return 'success';
  }
