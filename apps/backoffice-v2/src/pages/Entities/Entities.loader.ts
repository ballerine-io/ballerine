...
export const entitiesLoader: LoaderFunction = async ({ request }) => {
  const { entity, filterId, sortBy, sortDir } = qs.parse(request.url.split('?')[1]);
  ...
  if (!entity || !filterId || !session?.user || !sortBy || !sortDir) return null;

  const usersList = usersQueryKeys.list();
  await queryClient.ensureQueryData(usersList.queryKey, usersList.queryFn);
  const workflowList = workflowsQueryKeys.list({ filterId, sortBy, sortDir });
