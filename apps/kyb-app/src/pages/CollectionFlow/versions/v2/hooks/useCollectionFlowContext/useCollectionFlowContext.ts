...
import { assignDocumentStatusesToDocumentsInContext } from './helpers/assign-document-statuses-to-documents-in-context';
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        setFinalContext(assignDocumentStatusesToDocumentsInContext(context, uiSchema, documents));
      } catch (error) {
