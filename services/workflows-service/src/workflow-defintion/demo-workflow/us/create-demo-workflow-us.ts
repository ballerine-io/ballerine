...
export const upsertDemoUsKybFlow = async (customerId: string) => {
  const prisma = new PrismaClient();

  try {
    return await prisma.$transaction(async (transaction: Prisma.TransactionClient) => {
      console.log(`Upserting US KYB demo flow for customer: ${customerId}`);
      ...
      try {
        ...
        const updatedConfig = {
          ...
          isDemoKybUsEnabled: true,
        };

        const updatedFeatures = {
          ...(customer.features as Record<string, unknown>),
          enableWebPresenceReportExport: true,
          mockDashboardV1: true,
        };
        ...
        const workflowDefinitionId = `${customerId}_kyb_demo_us`;
        const workflowName = 'KYB Onboarding Demo - US';

        console.log(`Upserting workflow definition with ID: ${workflowDefinitionId}`);

        const kybDemoUsDefinition = generateWorkflowDefinition({
        ...
        if (existingWorkflowDefinition) {
          workflowDefinition = await transaction.workflowDefinition
            .update({
              ...
              data: {
                name: kybDemoUsDefinition.name,
                version: kybDemoUsDefinition.version,
                definitionType: kybDemoUsDefinition.definitionType,
                definition: kybDemoUsDefinition.definition,
                extensions: kybDemoUsDefinition.extensions,
                config: kybDemoUsDefinition.config,
                crossEnvKey: kybDemoUsDefinition.crossEnvKey,
                contextSchema: kybDemoUsDefinition.contextSchema,
              },
            })
            .catch(error => {
        ...
        const uiDefinitionId = `${customerId}_kyb_demo_us_ui_definition`;
        ...
        const filterId = `${customerId}_kyb_demo_us_filter`;
        ...
        console.log(`US KYB demo flow upserted successfully for customer: ${customerId}`);
        ...
        console.log(`Error in transaction for customer ${customerId}:`, error);
        console.error(`Error in transaction for customer ${customerId}:`, error);
        throw error;
      }
    });
  } catch (error) {
    console.log(`Failed to upsert US KYB demo flow for customer ${customerId}:`, error);
    console.error(`Failed to upsert US KYB demo flow for customer ${customerId}:`, error);
    throw error;
  } finally {
