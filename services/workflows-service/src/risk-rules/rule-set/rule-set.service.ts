...
export class RuleSetService {
  ...
  async findAssociatedRiskPolcies(rulesetId: string, projectIds: TProjectIds) {
    const ruleSetWithParents =  await this.ruleSetRepository.findAllAssociatedPolicies(rulesetId, projectIds) as RuleSetWithParent;
    const riskRulePolicy = extractRiskRulePolicy(ruleSetWithParents);

    return riskRulePolicy;
  }

  async assignRuleSetToParentRuleset(
  ...
  async unassignRuleFromRuleset(
    ...
    return await this.ruleSetRepository.disconnectFromRuleset(
      ruleSetId,
      parentRuleSetId,
    );
  }
